...
RAG + Chain-of-Thought (CoT) を使って質問を生成する（パターン5・6）。
結果は qg_result.json に追記される。

処理フロー:
  1. チャンクのOCR+ASRからキーワード（seed）をLLMで抽出
  2. seedでRAG類似度検索
  3. RAG結果をもとにLLMが詳細な推論（CoT）を生成
  4. CoT＋RAG結果から質問・解答を生成

使い方:
  # パターン5: スライド枚数基準（3枚ごとに1問）
  python qg/qg_rag_cot.py movie/lecture/result.json \
      --chroma movie/lecture/chroma_char_asr_ocr_mix \
      --by-slides

  # パターン6: 経過時間基準（5分ごとに1問）
  python qg/qg_rag_cot.py movie/lecture/result.json \
      --chroma movie/lecture/chroma_char_asr_ocr_mix \
      --by-time
...
COT_PROMPT = """\
以下の講義資料とキーワードをもとに、この講義内容の重要なポイントについて分析してください。
...
以下の観点を箇条書きで簡潔に記述してください：
1. 主要な概念と概念間の関係性
2. 問題として問えること（{bloom_levels}の観点）"""

QG_COT_PROMPT = """\
以下の推論・分析結果と講義資料をもとに、講義内容を理解できているか確認するための4択問題を{n}問生成してください。
...
def generate_cot(
    ...
    try:
        ...
        print(f"[ERROR] CoT生成失敗: {e}")
        return ""


def generate_questions_with_cot(
    seeds: list[str],
    context: str,
    cot_text: str,
    n: int,
    ...
    if not cot_text or not context.strip():
        return []
    ...
    payload = {
        "model": model,
        "messages": [{"role": "user", "content": QG_COT_PROMPT.format(
            n=n,
            cot_text=cot_text,
            seeds="、".join(seeds),
            context=context,
...
def run(
    ...
    for chunk in chunks:
        ...
        # Step 3: CoT推論
        cot_text = generate_cot(seeds, context, model, bloom_level, proper_nouns, lmstudio_url)
        if not cot_text:
            print("SKIP (CoT生成失敗)")
            continue
        print(f"CoT:{len(cot_text)}文字", end=" ", flush=True)
        ...
        # Step 4: 質問生成（CoT付き）
        questions = generate_questions_with_cot(
            seeds, context, cot_text, questions_per_chunk, model, bloom_level, proper_nouns, lmstudio_url,
        )
        if questions:
            for qi, q in enumerate(questions):
                qid = f"rag_cot_{chunk_mode}_{chunk['chunk_id']}_q{qi:02d}"
                new_entries.append({
                    "question_id":    qid,
                    "model":          model,
                    "method":         "rag_cot",
    ...
    if save_chunks:
        chunks_path = Path(result_json_path).parent / f"qg_chunks_{chunk_mode}_cot.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="RAG + CoT で質問生成（パターン5・6）",
    ...
    mode_group.add_argument("--by-slides", action="store_true", help="スライド枚数基準（パターン5）")
    mode_group.add_argument("--by-time",   action="store_true", help="経過時間基準（パターン6）")
    ...
    parser.add_argument("--slide-overlap",        type=int,   default=0,   help="スライド枚数基準の被り枚数")
    parser.add_argument("--time-overlap",         type=float, default=0.0, help="経過時間基準の被り秒数")
    ...
    parser.add_argument("--save-chunks",     action="store_true",                help="LLMに渡すテキスト・CoTを別ファイルに保存して確認する")
