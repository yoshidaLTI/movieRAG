...
文字数チャンキングRAGインデックスに対してクエリを実行する。

使い方:
  python rag_search.py "TCP/IPとは？" --chroma movie/test1/chroma_char_asr_ocr_mix
  python rag_search.py "TCP/IPとは？" --chroma movie/test1/chroma_char_ocr
  python rag_search.py "TCP/IPとは？" --chroma movie/test1/chroma_char_asr --k 5

時間的整合ありモード（_asr/_ocr ペアを相互逆引き）:
  python rag_search.py "TCP/IPとは？" --chroma movie/test1/chroma_char_asr --aligned
  python rag_search.py "TCP/IPとは？" --chroma movie/test1/chroma_char_ocr --aligned
"""

import argparse
from pathlib import Path

from rag.retriever import load_stores, search, search_aligned
...
def main():
    parser = argparse.ArgumentParser(description="文字数チャンキングRAG 検索")
